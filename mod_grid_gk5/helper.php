...
class GridGK5Helper {
	...
	public function moduleCSS() {
		...
		for($i = 0; $i < count($block_data); $i++) {
			$el = $block_data[$i];
			$output_desktop .= $prefix . str_replace(array('[', ']'), array('_', ''), $el->ID) . ' { height: '.str_replace(',', '.', ($el->SIZE_D_H * (100.0 / $mod_height_desktop))).'%; width: '.str_replace(',', '.', ($el->SIZE_D_W * (100.0 / 6))).'%; left: '.str_replace(',', '.', ($el->POS_D_X * (100.0 / 6))).'%; top: '.str_replace(',', '.', ($el->POS_D_Y * (100.0 / $mod_height_desktop))).'%; z-index: '.($i+1).'; }' . "\n";
			$output_tablet .= $prefix . str_replace(array('[', ']'), array('_', ''), $el->ID) . ' { height: '.str_replace(',', '.', ($el->SIZE_T_H * (100.0 / $mod_height_tablet))).'%; width: '.str_replace(',', '.', ($el->SIZE_T_W * (100.0 / 4))).'%; left: '.str_replace(',', '.', ($el->POS_T_X * (100.0 / 4))).'%; top: '.str_replace(',', '.', ($el->POS_T_Y * (100.0 / $mod_height_tablet))).'%; z-index: '.($i+1).'; }' . "\n";
			$output_mobile .= $prefix . str_replace(array('[', ']'), array('_', ''), $el->ID) . ' { height: '.str_replace(',', '.', ($el->SIZE_M_H * (100.0 / $mod_height_mobile))).'%; width: '.str_replace(',', '.', ($el->SIZE_M_W * (100.0 / 2))).'%; left: '.str_replace(',', '.', ($el->POS_M_X * (100.0 / 2))).'%; top: '.str_replace(',', '.', ($el->POS_M_Y * (100.0 / $mod_height_mobile))).'%; z-index: '.($i+1).'; }' . "\n";
		}
